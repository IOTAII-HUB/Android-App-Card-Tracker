...
public class Phone extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Set a darker status bar color
        Window window = getWindow();
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            window.setStatusBarColor(getResources().getColor(android.R.color.darker_gray, getTheme()));
        ...
        // Check if a phone number is stored in SharedPreferences
        SharedPreferences sharedPreferences = getSharedPreferences(PREF_NAME, Context.MODE_PRIVATE);
        String savedPhoneNumber = sharedPreferences.getString(PREF_PHONE_NUMBER, null);
        if (savedPhoneNumber != null) {
            // If a phone number is stored, automatically navigate to SecondActivity
            Intent intent = new Intent(Phone.this, SecondActivity.class);
            intent.putExtra("phoneNumber", savedPhoneNumber);
            intent.putExtra("userId", userId);
            startActivity(intent);
            finish(); // Finish the current activity to prevent going back to it when pressing back from SecondActivity
        }
        ...
        submitButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (selectedDevice.equals(DEFAULT_SELECTION)) {
                    Toast.makeText(Phone.this, "Please select a device", Toast.LENGTH_SHORT).show();
                    return;
                }
                ...
                // Save the phone number in SharedPreferences for future use (remember me)
    ...
    private void updatePhoneNumber(String userId, String deviceId, String phoneNumber) {
        ...
        new Thread(() -> {
            try {
                ...
                if (responseCode == HttpURLConnection.HTTP_OK) {
                    runOnUiThread(() -> {
                        Toast.makeText(Phone.this, "Phone number updated successfully", Toast.LENGTH_SHORT).show();
                    });
                } else {
                    runOnUiThread(() -> {
                        Toast.makeText(Phone.this, "Failed to update phone number", Toast.LENGTH_SHORT).show();
                    });
                }
                connection.disconnect();
                ...
                e.printStackTrace();
                runOnUiThread(() -> {
                    Toast.makeText(Phone.this, "Error: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                });
            }
        }).start();
    }
